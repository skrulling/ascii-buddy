export class ExportManager {
    ...
    downloadPNG(asciiGrid) {
        if (asciiGrid.length === 0 || asciiGrid[0].length === 0)
            return;
        ...
        for (let y = 0; y < asciiGrid.length; y++) {
            for (let x = 0; x < asciiGrid[y].length; x++) {
                ctx.fillText(asciiGrid[y][x], padding + x * charWidth, padding + y * charHeight);
            }
        }
        // Download
        canvas.toBlob((blob) => {
